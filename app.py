...
# apply method to create icrcle to tk.Canvas
    ...
    if add_charge:
        if len(h.charges) > 9:
            messagebox.showerror(
                title="Zu viele Ladungen", message="Es können maximal 10 Ladungen eingefügt werden.")
        else:
            ...
            h.charges.append(
                h.charge(1, [(coords[0] + coords[2]) / 2, (coords[1] + coords[3]) / 2]))
...
removecharge_button = tk.Button(
    buttons_f, text="Ladung entfernen", width=c.BUTTON_WIDTH, command=onClick_removeCharge)
